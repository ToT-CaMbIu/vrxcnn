...
void opencl_create_program_conv(CLVars& cl_vars,
    ...
    cl_mem Filter_cl = clCreateBuffer(cl_vars.context, CL_MEM_READ_ONLY, n1 * m1 * sizeof(float),
                                         NULL, &cl_vars.clStatus);
    cl_mem C_cl = clCreateBuffer(cl_vars.context, CL_MEM_WRITE_ONLY, n2 * m2 * sizeof(float), NULL,
                                    &cl_vars.clStatus);
    ...
    clSetKernelArg(cl_vars.kernel, 12, ts * ts * sizeof(float), NULL);
    clSetKernelArg(cl_vars.kernel, 13, ts * ts * sizeof(float), NULL);
    ...
    cl_vars.clStatus |= clEnqueueReadBuffer(cl_vars.command_queue, C_cl, CL_TRUE, 0,
                                           n2 * m2 * sizeof(float), C, 0, NULL, NULL);
...
std::vector<float> make_convolution(CLVars& cl_vars) {
     ...
     //input parameters
     int n = rand() % 4000 + 1000, m = rand() % 4000 + 1133;
     int n1 = 3, m1 = 3;
     ...
     //could be possible to reduce count of the parameters
